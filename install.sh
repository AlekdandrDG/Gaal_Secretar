...
# Refuse to run as root — installs files inside the project dir, must be owned by your user
if [ "$EUID" -eq 0 ]; then
    echo "ERROR: Do not run install.sh as root."
    echo "Run as a regular user; the script uses sudo only for individual system commands."
    exit 1
fi
