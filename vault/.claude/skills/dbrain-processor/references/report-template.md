...
<b>🎯 Закрыто сегодня:</b> {K}
• {task_name} <i>(закрыта в {time})</i>

<b>🗑 Удалено из Todoist:</b> {D}
• 🤝 {meeting} — отменена <i>(укажи причину ответом, если важно)</i>
• 🗑 {task} — удалена

<b>❓ Уточнить людей:</b> {Q}
• «{как услышано}» — {кандидат}? <i>(ответь — привяжу)</i>
...
### Deleted (🗑)
Сравни вчерашний снэпшот `vault/.snapshots/tasks-*.json` с текущим Todoist (см. SKILL § Deleted Tasks Tracking).
Удалённые = были вчера, нет сейчас И не закрыты сегодня.
- Встречи (float/созвон/звонок с человеком) → 🤝, попроси причину, запиши в people/CRM-карточку.
- Прочее → 🗑, фиксируй факт.
Показывать секцию только если есть удалённые.
