...
`vault/CORE.md` — ядро, которое читается ПЕРВЫМ в каждой сессии. Правила:

- **Лимит: 1200 знаков.** Превысил — сожми, убрав наименее долговечное.
- Содержимое: кто пользователь, работа, ≤3 активные цели, ключевые люди (3-5), устойчивые предпочтения.
- **Переписывать, не дописывать**: изменился факт (новая цель, смена работы) → замени старый.
- Обновляй ТОЛЬКО при реальном изменении долговечных фактов. Обычный день = не трогать.
- Однодневные события, задачи, детали встреч — сюда НЕ попадают (им место в daily/карточках).
