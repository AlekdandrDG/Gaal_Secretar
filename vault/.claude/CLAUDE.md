...
0. `vault/CORE.md` — ядро (≤1200 знаков): кто пользователь, цели, предпочтения. ВСЕГДА первым.
...
## MEMORY ACCESS RULES (ОБЯЗАТЕЛЬНО при ответах о прошлом)

1. **Гибрид всегда.** Вопрос про период («что было на той неделе / в июне») → прочитай выжимки `summaries/` за период **И** сделай поиск (grep/чтение) по `daily/` за тот же период. Обе ноги, никогда только выжимка.
2. **Drill-down.** Конкретные факты — суммы, даты, имена, формулировки — бери ТОЛЬКО из сырых `daily/` или карточек. Выжимка — карта, не цитаты. В каждой выжимке есть блок `## Источники` со ссылками на дни — открывай их.
3. **Карточки — первоисточник статусов.** Договорённости → `agreements/` (актуальный статус во frontmatter, история изменений в `## History`), встречи → `meetings/`, люди → `people/`. Противоречие карточки и выжимки → верь карточке.
4. **Не уверен → слой ниже.** Мало деталей в выжимке — спустись в daily. Это дёшево, галлюцинация — дорого.
...
| Folder | Purpose |
|--------|---------|
| `daily/` | Raw daily entries (YYYY-MM-DD.md) — append-only, источник правды |
| `summaries/` | Свёртки: `YYYY-Wnn-summary.md` (недели), `YYYY-MM-summary.md` (месяцы), в конце каждой — `## Источники` |
| `agreements/` | Договорённости/обязательства: status + append-only `## History` |
| `meetings/` | Состоявшиеся встречи: участники, итоги, суммы, next steps |
